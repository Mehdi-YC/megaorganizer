...
<div class="relative h-full w-full overflow-hidden rounded-sm border border-border bg-bg-subdued">
	<div class="absolute top-2 right-2 z-10 flex gap-1">
		<button
			type="button"
			class="flex h-7 w-7 cursor-pointer items-center justify-center rounded-sm border border-border bg-surface text-xs text-fg-subdued transition-colors hover:bg-muted hover:text-fg"
			onclick={fitView}
			title="Fit view"
			aria-label="Fit view"
		>
	...
	>
		...
		{#each edges as edge (edge.from.x + edge.from.y + '-' + edge.to.x + edge.to.y)}
			<path
				d="M {edge.from.x} {edge.from.y} C {edge.from.x} {edge.from.y + 30}, {edge.to.x} {edge.to
					.y - 30}, {edge.to.x} {edge.to.y}"
				fill="none"
		...
		{#each flatNodes as nd (nd.id)}
			...
			{#if pos}
				...
				<g
					...
					onclick={(e) => {
						e.stopPropagation();
						if (!hasDragged) onNodeClick(nd.node);
					}}
				>
					<rect
						width={NODE_W}
						height={NODE_H}
						rx="4"
						class="fill-surface stroke-border transition-colors hover:stroke-primary"
						stroke-width="1.5"
					/>
	...
	{#if flatNodes.length === 0}
		<div class="absolute inset-0 z-20 flex items-center justify-center p-4">
			<EmptyState icon="fa-diagram-project" message="No children to display" />
		</div>
	{/if}
</div>
