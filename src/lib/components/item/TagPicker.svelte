...
<div>
	<div class="mb-3 flex items-center justify-between">
		<h3 class="text-xs font-semibold tracking-wide text-fg-accent uppercase">Tags</h3>
		{#if editing}
			<Button size="sm" variant="ghost" onclick={() => (showPicker = !showPicker)}>
	...
	{#if showPicker && editing}
		<div class="mb-3 max-h-40 space-y-1 overflow-y-auto">
			{#each allTags as t (t.id)}
				<button
					type="button"
					class="flex w-full cursor-pointer items-center gap-2 rounded-sm px-2 py-1.5 text-left text-xs transition-colors hover:bg-muted {tagIds.includes(
						t.id
					)
						? 'bg-muted'
						: ''}"
			...
			{:else}
				<p class="text-[11px] text-fg-subdued italic">
					No tags. Create some in <a href="/app/tags" class="text-primary hover:text-primary-hover"
						>Tags</a
					> page.
				</p>
			{/each}
