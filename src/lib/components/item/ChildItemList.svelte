<script lang="ts">
	...
	async function searchToAdd() {
		if (!addQuery.trim() || addQuery.trim().length < 3) {
			addResults = [];
			return;
		}
		const results = await searchTree(addQuery, 'item');
		addResults = results.filter(
			(r: any) => r.id !== parentId && !children.some((c) => c.id === r.id)
		);
	}
...
<div class="rounded-sm border border-border bg-surface p-4 sm:p-5">
	...
	{#if children.length > 0}
		<div class="space-y-0.5">
			{#each children as child, idx (child.id)}
				<div
					class="group flex cursor-move items-center gap-2 rounded-sm px-2 py-1.5 text-xs hover:bg-muted"
					draggable="true"
					role="listitem"
				...
				>
					<i
						class="fas fa-grip-vertical w-3 text-center text-fg-subdued/40 transition-opacity sm:opacity-0 sm:group-hover:opacity-100"
					></i>
					<a href="/app/item/{child.id}" class="flex flex-1 items-center gap-2 text-fg">
						{#if child.imageUrl}
							<img src={child.imageUrl} alt="" class="h-5 w-5 shrink-0 rounded-sm object-cover" />
						{:else}
					...
					<button
						type="button"
						class="flex h-5 w-5 cursor-pointer items-center justify-center rounded-sm text-fg-subdued hover:text-error sm:hidden sm:group-hover:flex"
	...
	{:else if !showAdd}
		<p class="py-4 text-center text-xs text-fg-subdued">No child items yet.</p>
	{/if}
</div>
