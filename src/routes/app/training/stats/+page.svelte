<script lang="ts">
	...
	function getRunningDistanceData() {
		...
		return runningData.map((r) => ({
			label: new Date(r.startedAt).toLocaleDateString('default', {
				month: 'short',
				day: 'numeric'
			}),
	...
	function formatMinutes(v: number) {
		return String(v);
	}
	...
	let weeklyChart = $derived(
		(() => {
			...
			return {
				...
				max,
				ticks: getYTicks(max),
				path: buildSvgPath(values, 530, 150, 0, max),
				area: buildSvgArea(values, 530, 150, 0, max)
			};
		})()
	);
...
<div class="max-w-5xl p-4 sm:p-8">
	<div class="mb-6">
	...
	<h1 class="mb-6 text-lg font-semibold text-fg-accent">Training Stats</h1>

	{#if sessions.length === 0}
		<EmptyState
			icon="fa-chart-line"
			message="No completed sessions yet"
			submessage="Start your first session to see stats"
		/>
	{:else}
		<AreaChart
		...
		<div class="rounded-sm border border-border bg-surface p-5">
			<h2 class="mb-4 text-xs font-semibold tracking-wide text-fg-accent uppercase">
				Session Types
			</h2>
			<div class="space-y-2">
